      ...
      <Link href={`/product/${product.id}`} className="block h-full flex flex-col">
        ...
        <CardContent className="p-4 flex-grow">
          ...
          <div className="flex items-center justify-between">
            <p className="text-xl font-bold text-primary">{product.price.toFixed(2)} ر.س</p>
            {product.rating && (
          ...
          {product.originalPrice && (
            <p className="text-sm text-slate-400 line-through mt-1">{product.originalPrice.toFixed(2)} ر.س</p>
          )}
        </CardContent>
        <CardFooter className="p-4 pt-0 mt-auto">
          <Button
            size="sm"
            className={`w-full font-semibold rounded-lg h-10 transition-all duration-200 ${
              isAdded 
